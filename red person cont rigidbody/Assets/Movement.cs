...
public class Movement : MonoBehaviour
{
	...
	private readonly float m_fRadius = 1;
	...
	private void Move()
	{
		...
		if (m_fCurrSpeedMag > m_fMaxSpeed * m_fMaxSpeed == false)
		{
	...
	private void Jump()
	{
		if (Input.GetButtonDown("Jump") && GroundCheck() == true)
		{
			...
			m_bDoubleJumpOn = true;
		}
		else
		{
			if (Input.GetButtonDown("Jump") && m_bDoubleJumpOn)
			{
				m_bDoubleJumpOn = false;
				m_rbRigidbody.AddForce(Vector3.up * m_fJumpPower * Time.fixedDeltaTime, ForceMode.VelocityChange);
			}
		}
